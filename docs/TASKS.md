...
- [ ] **P6-T45** Driver-aware lineup count in TournamentProfileData (JSONB_ARRAY_LENGTH pgsql / JSON_ARRAY_LENGTH sqlite / JSON_LENGTH mysql) — fixes SQLSTATE crash after tournament creation — PR #200
- [ ] **P6-T46** team_id stored on individual-event participations (resolved from active team membership; backfill migration); team-vs-individual classification moved to events.event_type; individual medals now reach coach achievements — PR #201
