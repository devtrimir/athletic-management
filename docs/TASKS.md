...
- [ ] **P0-T04** Add remaining NPM packages (`recharts`; verify pre-installed: `lucide-react`, `clsx`, `tailwind-merge`, `class-variance-authority`, `babel-plugin-react-compiler`) — forms use Inertia `useForm`, ad-hoc requests use Inertia `useHttp` (no react-hook-form / zod / TanStack Query)
- [ ] **P0-T05** Self-host fonts (Noto Sans Devanagari, Inter) under `resources/fonts/`; wire into Tailwind v4 `@theme`
- [ ] **P0-T06** Project config files (`.editorconfig`, `.prettierrc`, `eslint.config.js` flat, `phpstan.neon` (Larastan level 6), `pint.json` already present — verify). Also: bump `composer.json` `php: ^8.3 → ^8.4`; rename package; confirm `.env` `DB_CONNECTION=mysql` + `mysql.collation=utf8mb4_0900_ai_ci`.
...
- [ ] **P0-T12** Inertia `SetLocale` middleware stub + `/{locale}` URL prefix (default `hi`)
