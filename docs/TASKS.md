...
- [x] **P5-T19** Tournaments export FE: bulk-select + Export dialog in `tournaments/index.tsx`
...
- [x] **P6-T30** Remove coach `nis_certified` check end-to-end (migration, model, requests, exports, print, listing/show/create/edit UI, i18n, tests); `nis_master_id` retained — PRs #180 (merged) + #181 (revisions: restore `nis_master_id`, show NIS info on profile)
...
- [x] **P6-T52** Cross-module guards: block deleting tournament/event/participation/medal when used as coach or member promotion/reward evidence — PR #209
- [x] **P6-T53** Fix missing Hindi/English translations and localize enum labels on team prabhari (incharge) listing, profile, print, and team assignment panel — PR #210
- [~] **P6-T54** Remove designation field from incharge module and team incharge surfaces (forms, listing, show, print, team panel) — PR #211
...
- [x] **P6-T33** Remove duplicate 'Coach Listing' h1 header from the coach listing print page (letterhead subtitle already carries the report title) — PR #184
- [x] **P6-T32** Remove coach designation fields (`designation`, `designation_master_id`, `designation_en`) end-to-end and drop columns; add NIS info column to active coaches listing — PR #183
- [x] **P6-T31** Coach certification document upload: file input in add/edit dialog (pdf/jpeg/png/webp, 5 MB), private `local` disk storage with meta columns, per-row "View document" button opening the shared `ConfidentialDocumentPreview` modal with download — PR #182
...
- [x] **P7-T05** Service: ResignationDismissalLogReport
...
- [x] **P7-T10** `ReportController` Inertia (`Index`, `Show`)
- [x] **P7-T11** `ExportReportJob` (XLSX via `maatwebsite/excel`)
