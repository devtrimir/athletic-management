...
function buildPrintHtml(
    ...
    const statusRows = data.status_history
        .map(
            (h) =>
                `<tr><td>${formatDate(h.effective_on, '—')}</td><td>${t(h.status)}</td><td>${h.reason ?? '—'}</td></tr>`,
        )
        .join('');
    ...
    const teamRows = data.team_history
        .map(
            (th) =>
                `<tr><td>${th.team_name ?? '—'}</td><td>${th.session_name ?? '—'}</td><td>${t(th.role)}</td><td>${formatDate(th.joined_on, '—')}</td><td>${formatDate(th.left_on, t('Present'))}</td></tr>`,
        )
        .join('');
    ...
    const achievementRows = data.achievements
        .map(
            (a) =>
                `<tr><td>${t(a.level)}</td><td>${a.competition_details}</td><td>${a.event ?? '—'}</td><td>${a.medal_type ? t(a.medal_type) : '—'}</td><td>${formatDate(a.event_date, '—')}</td></tr>`,
        )
        .join('');
    ...
    ${row(t('Date of birth'), formatDate(data.dob))}
    ...
    ${row(t('Joining date'), formatDate(data.joining_date))}
    ${row(t('Promotion date'), formatDate(data.promotion_date))}
                        ...
                        <div className="py-2">
                            ...
                            <Section title={t('Service')}>
                                ...
                                <InfoRow
                                    ...
                                    value={formatDate(data.joining_date)}
                                />
                                <InfoRow
