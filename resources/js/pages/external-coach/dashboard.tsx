            ...
            <main className="min-h-screen bg-muted/20">
                <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:gap-6 sm:px-6 lg:py-8">
                        ...
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="min-w-0">
                                ...
                                <p className="mt-1 text-sm text-muted-foreground">
                                    {t(
                                        'Review assigned athletes and submit attendance or performance updates.',
                                    )}
                                </p>
                            </div>
                    ...
                    <section className="grid grid-cols-3 gap-2 sm:gap-3">
                        <SummaryTile
                            icon={Users}
                            label={t('Assigned athletes')}
                            value={summary.active_assignments}
                        />
                        <SummaryTile
                            icon={Activity}
                            label={t('Sports covered')}
                            value={summary.sports_covered}
                        />
                        <SummaryTile
                            icon={MapPin}
                            label={t('Training venues')}
                            value={summary.training_venues}
                        />
                    </section>
                        ...
                        <div className="flex flex-col gap-3 border-b px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {t(
                                        'Active external training assignments available for attendance and performance updates.',
                                    )}
                                </p>
                            </div>
                            <form
                                action="/external-coach/dashboard"
                                method="get"
                                className="grid gap-2 sm:w-72 sm:grid-cols-[minmax(0,1fr)_auto]"
                            >
                                <Input
                                    name="pno"
                                    defaultValue={filters.pno}
                                    placeholder={t('Search PNO')}
                                    inputMode="search"
                                    className="h-9"
                                />
                                <Button
                        ...
                        {assignments.data.length > 0 ? (
                            <div className="divide-y">
                                {assignments.data.map((assignment) => (
                                    <article
                                        key={assignment.id}
                                        className="grid gap-4 px-4 py-4 sm:px-5 xl:grid-cols-[minmax(0,1fr)_minmax(260px,340px)_auto] xl:items-center"
                                    >
                                        <AssignedAthlete
                                        ...
                                        <div className="grid grid-cols-3 gap-2 xl:flex xl:justify-end">
                                            ...
                                            >
                                                <Link
                                                    href={`/external-coach/athletes/${assignment.member.id}`}
                                                >
                                            ...
                                            >
                                                <Link
                                                    href={`/external-coach/performance?assignment=${assignment.id}`}
                                                >
                                                    <TrendingUp className="size-4" />
                                                    {t('Update')}
                                                </Link>
                                            ...
                                            >
                                                <Link
                                                    href={`/external-coach/attendance?assignment=${assignment.id}`}
                                                >
                                                    <ClipboardCheck className="size-4" />
                                                    {t('Mark')}
                                                </Link>
                            ...
                            <div className="px-4 py-10 text-center sm:px-5">
                                ...
                                <h3 className="mt-3 text-sm font-medium">
                                    {t('No assigned athletes')}
                                </h3>
                                <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
                                    {t(
                                        'Active assignments created by the sports unit will appear here for attendance and performance reporting.',
                                    )}
                                </p>
                            </div>
                        )}
    ...
    return (
        <div className="rounded-lg border bg-card p-3 shadow-sm sm:p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                <div className="min-w-0">
                    <div className="truncate text-[11px] leading-tight font-medium text-muted-foreground sm:text-xs">
                        {label}
                    </div>
    ...
    return (
        <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge
                    variant="outline"
                    className={memberStatusBadgeClass(
                        assignment.member.current_status ?? 'unknown',
                    )}
                >
...
function AssignmentSchedule({ assignment }: { assignment: Assignment }) {
    ...
    const time = [assignment.training_start_time, assignment.training_end_time]
        .filter(Boolean)
        .join(' - ');
