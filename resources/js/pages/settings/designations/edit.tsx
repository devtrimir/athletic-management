...
type Designation = { id: number; code: string; name: string; short_name: string | null; designation_order: number; mapped_rank_code: string | null; designation_type: string | null; is_active: boolean; };

export default function Edit({ designation, ranks }: { designation: Designation; ranks: Rank[] }) {
    const { t } = useTranslation();
    setLayoutProps({ breadcrumbs: [{ title: 'Designations', href: DesignationController.index.url() }, { title: designation.name, href: DesignationController.edit.url(designation.id) }] });
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading variant="small" title={`${t('Edit')} ${designation.name}`} description={t('Update designation reference details')} />
                    ...
                    {({ processing, errors }) => (
                        <>
                            <div className="rounded-xl border bg-card p-6 space-y-5">
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    <div className="grid gap-2"><Label htmlFor="name">{t('Name')}</Label><Input id="name" name="name" defaultValue={designation.name} required maxLength={255} /><InputError message={errors.name} /></div>
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    <div className="grid gap-2"><Label htmlFor="mapped_rank_code">{t('Mapped rank')}</Label><Select name="mapped_rank_code" defaultValue={designation.mapped_rank_code ?? ''}><SelectTrigger id="mapped_rank_code"><SelectValue placeholder={t('Select rank')} /></SelectTrigger><SelectContent>{ranks.map((rank) => <SelectItem key={rank.id} value={rank.code}>{rank.name}</SelectItem>)}</SelectContent></Select><InputError message={errors.mapped_rank_code} /></div>
                                </div>
