    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading variant="small" title={t('New designation')} description={t('Add a new designation reference record')} />
                <Form {...DesignationController.store.form()} className="max-w-2xl space-y-6">
                    {({ processing, errors }) => (
                        <>
                            ...
                            <div className="flex items-center gap-3"><Button disabled={processing}>{t('Create designation')}</Button><Button variant="outline" asChild><Link href={DesignationController.index.url()}>{t('Cancel')}</Link></Button></div>
                        </>
                    )}
                </Form>
            </div>
...
Create.layout = { breadcrumbs: [{ title: 'Designations', href: DesignationController.index.url() }, { title: 'New designation' }] };
