...
type Rank = { id: number; code: string; name: string; short_name: string | null; rank_order: number; cadre_type: string | null; is_gazetted: boolean; aliases: string[] | null; is_active: boolean; };

export default function Edit({ rank }: { rank: Rank }) {
    const { t } = useTranslation();
    setLayoutProps({ breadcrumbs: [{ title: 'Ranks', href: RankController.index.url() }, { title: rank.name, href: RankController.edit.url(rank.id) }] });
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading variant="small" title={`${t('Edit')} ${rank.name}`} description={t('Update rank reference details')} />
                    ...
                    {({ processing, errors }) => (
                        <>
                            <div className="rounded-xl border bg-card p-6 space-y-5">
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    <div className="grid gap-2"><Label htmlFor="name">{t('Name')}</Label><Input id="name" name="name" defaultValue={rank.name} required maxLength={255} /><InputError message={errors.name} /></div>
