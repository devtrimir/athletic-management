...
export default function Edit({ rank }: { rank: Rank }) {
    ...
    setLayoutProps({
        breadcrumbs: [
            ...
            { title: rank.name, href: RankController.edit.url(rank.id) },
        ],
    });
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading
                    variant="small"
                    title={`${t('Edit')} ${rank.name}`}
                    description={t('Update rank reference details')}
                />
                <Form
                    {...RankController.update.form(rank.id)}
