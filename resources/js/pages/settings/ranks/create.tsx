    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading variant="small" title={t('New rank')} description={t('Add a new rank reference record')} />

                <Form {...RankController.store.form()} className="max-w-2xl space-y-6">
                    {({ processing, errors }) => (
                        <>
                            <div className="rounded-xl border bg-card p-6 space-y-5">
                                <div className="grid gap-5 sm:grid-cols-2">
                                    <div className="grid gap-2"><Label htmlFor="code">{t('Code')}</Label><Input id="code" name="code" required maxLength={50} /><InputError message={errors.code} /></div>
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    <div className="grid gap-2"><Label htmlFor="is_gazetted">{t('Gazetted')}</Label><Select name="is_gazetted" defaultValue="0"><SelectTrigger id="is_gazetted"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="0">{t('No')}</SelectItem><SelectItem value="1">{t('Yes')}</SelectItem></SelectContent></Select><InputError message={errors.is_gazetted} /></div>
                                    <div className="grid gap-2"><Label htmlFor="is_active">{t('Active')}</Label><Select name="is_active" defaultValue="1"><SelectTrigger id="is_active"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="1">{t('Yes')}</SelectItem><SelectItem value="0">{t('No')}</SelectItem></SelectContent></Select><InputError message={errors.is_active} /></div>
                                </div>
                            </div>
                            <div className="flex items-center gap-3"><Button disabled={processing}>{t('Create rank')}</Button><Button variant="outline" asChild><Link href={RankController.index.url()}>{t('Cancel')}</Link></Button></div>
                        </>
                    )}
                </Form>
            </div>
...
Create.layout = { breadcrumbs: [{ title: 'Ranks', href: RankController.index.url() }, { title: 'New rank' }] };
