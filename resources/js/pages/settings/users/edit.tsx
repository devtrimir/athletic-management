            ...
            <div className="space-y-6">
                ...
                <form onSubmit={handleProfileSubmit} className="max-w-xl space-y-6">
                    <div className="space-y-5 rounded-xl border bg-card p-6">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Select
                                ...
                                onValueChange={(v) => profileForm.setData('locale', v)}
                            >
                    ...
                    <div className="space-y-3 rounded-xl border bg-card p-6">
                        <h3 className="text-sm font-medium text-muted-foreground">{t('Account status')}</h3>

                        <div className="flex items-center gap-2">
                            <Checkbox
                                id="is_active"
                                checked={profileForm.data.is_active}
                                onCheckedChange={(v) => profileForm.setData('is_active', Boolean(v))}
                            />
                    ...
                    <div className="space-y-5 rounded-xl border bg-card p-6">
                        <h3 className="text-sm font-medium text-muted-foreground">{t('Reset password')}</h3>
                        <p className="text-sm text-muted-foreground">
                            {t('Leave blank to keep the current password. Setting a new password will require the user to change it on next login.')}
                        </p>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    profileForm.setData('password_confirmation', e.target.value)
                                }
                                autoComplete="new-password"
                            />
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        <Button disabled={profileForm.processing}>{t('Save changes')}</Button>
                ...
                {roles.length > 0 && (
                    <form onSubmit={handleRolesSubmit} className="max-w-xl">
                        ...
                        <div className="mt-4 flex items-center gap-3">
                            <Button disabled={rolesForm.processing}>{t('Update roles')}</Button>
                        </div>
                    </form>
                )}
            </div>
